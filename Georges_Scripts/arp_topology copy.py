...
def visualize_arp_topology(G, output_file="network_topology.png"):
    ...
    if router_nodes:
        ...
        for i, switch in enumerate(switch_nodes):
            ...
            node_positions[switch] = (1.5 * math.cos(angle), 1.5 * math.sin(angle))
        ...
        for i, server in enumerate(server_nodes):
            ...
            node_positions[server] = (3 * math.cos(angle), 3 * math.sin(angle))
        ...
        for i, endpoint in enumerate(endpoint_nodes):
            ...
            node_positions[endpoint] = (5 * math.cos(angle), 5 * math.sin(angle))
    else:
        # Fallback to spring layout if no router found
        node_positions = nx.spring_layout(G, k=0.15, iterations=20)
    
    # --- Fallback: assign positions to any nodes not already placed ---
    missing_nodes = set(G.nodes()) - set(node_positions.keys())
    if missing_nodes:
        fallback_positions = nx.spring_layout(G.subgraph(missing_nodes), k=0.15, iterations=20)
...
def main():
    ...
    # Define your PVE nodes by their IP addresses (update these as needed)
    pve_nodes = ["192.168.14.2", "192.168.14.3", "192.168.14.4"]
    
    # Extend the topology with ARP data from each PVE node
    G = extend_topology_with_pve(G, pve_nodes, community, version)
    
    # Optionally, you could recalculate or update your categories here if needed.
